...
#
# ``DOORMAT_QUIET`` drops exactly those manners and nothing else. The
...
# unset for the real thing, where being raised and focused is the point.
QUIET = _flag("DOORMAT_QUIET")
...
def scale_factor(reported=None):
    ...
    ``DOORMAT_SCALE`` overrides it, because rendering at 2x has to be
    ...
    """
    for candidate in (os.environ.get("DOORMAT_SCALE", "").strip(),
                      reported):
        try:
...
class Window:
    ...
    def __init__(self, width=1000, height=720, title="doormat",
                 scale=None, icon=None):
        # `icon` is `(width, height, rgba)` and only X11 can do anything with
        # it: Windows and macOS take theirs from the bundle rather than from
        # the running process. Every window accepts it anyway, so a caller
        # that has one need not ask which platform it is on first.
        del icon
    ...
    def on_callback_error(self, where, exc):
        import traceback
        print("doormat: error in %s handler: %r" % (where, exc))
        traceback.print_exc()
    ...
    # stays headless. See `Toplevel` in this package's __init__.
    toplevel_class = None
