...
def _open_display():
    ...
    if not (fmt.red_mask and fmt.green_mask and fmt.blue_mask):
        ...
        raise X11Unavailable(
            "the default visual is not TrueColor (depth %d); there is "
            "no colormap allocator here" % depth)
    ...
    for name in ("WM_PROTOCOLS", "WM_DELETE_WINDOW", "UTF8_STRING",
                 ...
                 "_NET_WM_ICON", "DOORMAT_SELECTION"):
...
class X11Window(Window):
    ...
    def __init__(self, width=1000, height=720, title="doormat",
                 icon=None):
        display = _open_display()
        super().__init__(width, height, title, icon=icon)
        x11 = _libs["x11"]
    ...
    def _set_icon(self, icon):
        """Put `icon` into ``_NET_WM_ICON``, so a window manager has something
        better than the generic 'this is a program' icon.

        `icon` is ``(width, height, rgba)``: straight pixels, not a file. This
        package decodes no images and ships no artwork -- a caller that wants
        an icon already has a PNG decoder and a brand, and a window library
        has neither. One canonical image is enough; the server scales it to
        whatever size the manager asks the property for.

        None means no icon, which is a window without a picture on it rather
        than a window that failed to open.
        """
        if self._closed or not icon:
            return
...
class X11Tk(X11Window):
    ...
    def __init__(self, *_args, **kwargs):
        super().__init__(**kwargs)
        self.tk = None  # Tk's own root has one, and callers check for it


# `Toplevel` in this package reads this off the master, so a popup opened
# from a real window is a real window and one opened from a headless root
# stays headless.
