...
# linter, the test collector and the backend chooser all depend on.
...
class Win32Window(Window):
    ...
    def __init__(self, width=1000, height=720, title="doormat",
                 owner=None, icon=None):
        _load()
        # Not in _load(). Process DPI awareness can be set exactly once and
        # never unset, and _load() also runs from available(), which the
        # chooser calls on backends it is only *considering*. Probing for one
