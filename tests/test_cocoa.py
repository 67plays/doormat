...
from doormat import window  # noqa: E402
...
def test_present_pushes_the_framebuffer():
    with _Session() as win:
        # A buffer for a 900x600 page, in the device pixels this display
        # actually has: the image the view ends up with is measured in
        # points, and the two only agree on a 1x screen.
        win.canvas = support.Canvas(*win.to_device(900, 600),
                                    bg=support.rgb("#123456"))
        win.present()
...
def test_the_frame_is_pushed_at_the_displays_own_resolution():
    ...
    the softness this is here to prevent. DOORMAT_SCALE forces the dense
    ...
    saved = os.environ.get("DOORMAT_SCALE")
    os.environ["DOORMAT_SCALE"] = "2"
    try:
        with _Session() as win:
            assert win.scale == 2.0, "the override never reached the window"
            # Allocated in device pixels, because that is the only size this
            # canvas has: a 900x600 page at 2x is these many real pixels, and
            # working out that multiplication is the caller's job, not the
            # window's.
        ...
        if saved is None:
            ...
            os.environ["DOORMAT_SCALE"] = saved
...
def test_clipboard_round_trips_through_nspasteboard():
    with _Session() as win:
        ...
        assert win.clipboard_get() == "doormat clipboard probe"
