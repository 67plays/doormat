...
from doormat.window import (QUIET, STATE_ALT, STATE_CONTROL,
                            STATE_SHIFT)
...
def live_a_quiet_window_does_not_take_the_keyboard():
    ...
    if not QUIET:
        print("  ..  quiet-window check needs DOORMAT_QUIET=1")
        return
    with _Session(640, 480) as win:
...
def live_present_puts_the_right_colours_on_the_server():
    ...
    with _Session(300, 200) as win:
        canvas = support.Canvas(300, 200, support.rgb("#3366cc"))
        win.canvas = canvas
        canvas.fill_rect(0, 0, 40, 40, support.rgb("#ff0000"))
        win.present()
...
def live_present_is_skipped_when_nothing_changed():
    with _Session(200, 120) as win:
        canvas = support.Canvas(200, 120, support.rgb("#101010"))
        win.canvas = canvas
        win.present()
        first = win._frame
        win.present()
        assert win._frame is first, "a clean canvas should not be re-packed"
        canvas.fill_rect(0, 0, 10, 10, support.rgb("#ffffff"))
        win.present()
...
def live_an_odd_width_does_not_smear():
    ...
    with _Session(333, 111) as win:
        canvas = support.Canvas(333, 111, support.rgb("#ffffff"))
        win.canvas = canvas
        canvas.fill_rect(0, 0, 333, 111, support.rgb("#00ff00"))
        canvas.fill_rect(330, 0, 333, 111, support.rgb("#0000ff"))
        canvas.fill_rect(0, 108, 320, 111, support.rgb("#ff0000"))
        win.present()
...
def live_expose_asks_for_another_frame():
    with _Session(200, 150) as win:
        win.canvas = support.Canvas(200, 150, support.rgb("#204060"))
        win.present()
        win._repaint = False
        event = x11.XEvent()
...
def live_a_resize_reaches_the_canvas():
    with _Session(400, 300) as win:
        win.canvas = support.Canvas(400, 300, support.rgb("#ffffff"))
        seen = []
...
# and nowhere else. DOORMAT_SCALE stands in for an Xft.dpi of 192: a CI
...
def live_a_dense_frame_lands_one_pixel_for_one():
    ...
    A rectangle covering the first 10 CSS pixels has to reach device pixel 19
    and stop before device pixel 21. With a buffer allocated in CSS pixels it
    covered ten device pixels and the server scaled nothing, so pixel 19 was
    ...
    with _DenseSession(320, 240) as win:
        ...
        canvas = support.Canvas(640, 480, support.rgb("#3366cc"))
        win.canvas = canvas
        pump(win)
        ...
        canvas.fill_rect(0, 0, 20, 20, support.rgb("#ff0000"))
        win.present()
...
def live_copying_claims_the_clipboard():
    with _Session() as win:
        win.clipboard_clear()
        win.clipboard_append("doormat clipboard probe")
        ...
        eq(win.clipboard_get(), "doormat clipboard probe")
    ...
    with _Session() as owner:
        ...
        try:
            ...
            request.xselectionrequest.property = \
                x11._state["DOORMAT_SELECTION"]
            owner._on_selection(request)
            x11._libs["x11"].XSync(owner._display, False)
            eq(other._read_property(x11._state["DOORMAT_SELECTION"]),
               "over the wire")
        finally:
            other.destroy()
