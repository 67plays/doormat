...
def test_present_puts_the_right_colour_in_the_right_corner():
    ...
    with _Session() as win:
        canvas = support.Canvas(900, 600, support.rgb("#123456"))
        win.canvas = canvas
        canvas.fill_rect(0, 0, 100, 100, support.rgb("#ff0000"))
        canvas.fill_rect(0, 500, 100, 600, support.rgb("#00ff00"))
        win.present()
...
def test_the_bitmap_header_describes_a_top_down_frame():
    with _Session() as win:
        win.canvas = support.Canvas(900, 600, support.rgb("#ffffff"))
        win.present()
...
def test_present_is_skipped_when_nothing_changed():
    with _Session() as win:
        canvas = support.Canvas(200, 100)
        win.canvas = canvas
        win.present()
        first = win._frame
        win.present()
        assert win._frame is first, "a clean canvas was converted again"
        canvas.fill_rect(0, 0, 10, 10, support.rgb("#ff0000"))
        win.present()
...
def test_a_paint_message_repaints_the_last_frame():
    ...
    with _Session() as win:
        win.canvas = support.Canvas(900, 600, support.rgb("#204060"))
        win.present()
...
# DOORMAT_SCALE forces it: the window is still created and messaged for
...
def test_a_dense_frame_lands_one_pixel_for_one():
    """The proof that nothing is stretched: a rectangle drawn at device
    pixel 20 has to arrive at device pixel 20. Clicks come back the other
    way through the scale -- getting the buffer right and the hit testing
    wrong would be worse than doing neither."""
    with _DenseSession() as win:
        canvas = support.Canvas(640, 480, support.rgb("#123456"))
        win.canvas = canvas
        # The rectangle a 10x10 CSS one covers at this scale.
        canvas.fill_rect(20, 20, 40, 40, support.rgb("#ff0000"))
        win.present()
...
def test_a_dpi_change_resizes_the_buffer_to_match():
    ...
    with _Session() as win:
        canvas = support.Canvas(*win.to_device(900, 600),
                                bg=support.rgb("#ffffff"))
        win.canvas = canvas
        win.present()
        before = win.scale
        rect = win32.RECT()
...
def test_clipboard_round_trips_through_the_windows_clipboard():
    with _Session() as win:
        win.clipboard_clear()
        win.clipboard_append("doormat clipboard probe é中")
        assert win.clipboard_get() == "doormat clipboard probe é中"
